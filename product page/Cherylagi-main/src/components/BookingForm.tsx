...
import { CarModel, BookingData } from "../types";

interface BookingFormProps {
  isOpen: boolean;
  onClose: () => void;
  type: "test" | "prebook"; // 'test' for test drive, 'prebook' for pre-booking
  models: CarModel[];
  activeModel: CarModel;
}

export default function BookingForm({ isOpen, onClose, type, models, activeModel }: BookingFormProps) {
  ...
  // Sync chosen model when active model changes or modal opens
  React.useEffect(() => {
    if (isOpen) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setIsSuccess(true);
      // Generate unique ticket ID (e.g. CHY-E5-5421)
      const modelCode = formData.carModel === "omoda-e5" ? "EV" : "ICE";
      const randomNum = Math.floor(1000 + Math.random() * 9000);
      setTicketId(`CHY-${modelCode}-${randomNum}`);
    }, 1800);
  };

  const selectedCar = models.find((m) => m.id === formData.carModel) || activeModel;
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-sm overflow-hidden shadow-2xl">
        ...
        <button
          onClick={onClose}
          className="absolute top-5 right-5 text-slate-500 hover:text-slate-900 bg-slate-50 border border-slate-200 p-2 rounded-sm transition-all hover:scale-105 z-10"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        {isLoading ? (
          ...
          <div className="py-24 px-8 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <p className="text-xs text-slate-500 max-w-xs leading-relaxed">
              Kami sedang mendaftarkan jadwal Anda ke jaringan dealer Chery Indonesia terdekat...
            </p>
          </div>
        ) : isSuccess ? (
          /* Success state */
          <div className="p-8 sm:p-12 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-xl sm:text-2xl font-extrabold text-[#1A1A1A] font-sans">
                {type === "test" ? "Uji Kendara Terjadwal!" : "Pre-Booking Berhasil!"}
              </h3>
              <p className="text-xs sm:text-sm text-slate-500 max-w-md mx-auto leading-relaxed">
                Terima kasih, **{formData.fullName}**. Pengajuan Anda telah berhasil didaftarkan secara prioritas di sistem Chery Indonesia.
              </p>
            </div>
            ...
            <div className="bg-slate-100 border border-slate-200 p-4 rounded-sm text-xs text-slate-600 max-w-md leading-relaxed flex items-center gap-3">
              ...
              <span>
                Konsultan Sales Chery kami akan menghubungi Anda dalam waktu 15 menit melalui WhatsApp untuk melakukan verifikasi jadwal dan mengirimkan petunjuk pembayaran / penjemputan.
              </span>
            </div>

            <button
              onClick={onClose}
              className="bg-white border border-slate-200 hover:border-slate-400 text-slate-700 hover:text-slate-950 font-bold px-6 py-2.5 rounded-sm text-xs uppercase tracking-wider shadow-sm transition-colors"
            >
              Tutup Jendela
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
            <div>
              <span className="text-[10px] text-[#DA291C] uppercase font-mono font-bold tracking-widest block mb-1">
                {type === "test" ? "Sistem Reservasi Uji Kendara" : "Prioritas Pre-Booking Unit"}
              </span>
              <h3 className="text-xl sm:text-2xl font-extrabold text-[#1A1A1A]">
                {type === "test" ? "Formulir Booking Test Drive" : "Formulir Booking Order Chery"}
              </h3>
              <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
                {type === "test" 
                  ? "Rasakan kenyamanan, performa, dan kecanggihan teknologi mengemudi aslinya secara gratis langsung di rumah Anda atau dealer terdekat."
                  : "Amankan antrean produksi Anda untuk OMODA & TIGGO terbaru. Biaya booking Rp 5 Juta dapat di-refund penuh jika terjadi pembatalan."
                }
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              {/* Car Selection Dropdown inside form */}
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-sm px-4 py-3 text-sm text-slate-800 focus:outline-none focus:border-[#DA291C] font-sans"
                >
                  {models.map((m) => (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className="w-full bg-slate-50 border border-slate-200 rounded-sm pl-12 pr-4 py-3 text-sm text-slate-800 focus:outline-none focus:border-[#DA291C] font-sans"
                    />
                  </div>
                </div>
              ...
              {/* Grid 3 (Only shown for Test Drive bookings): Date & Time selection */}
              {type === "test" && (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className="w-full bg-slate-50 border border-slate-200 rounded-sm pl-12 pr-4 py-3 text-sm text-slate-800 focus:outline-none focus:border-[#DA291C] font-sans cursor-pointer"
                      />
                    </div>
                  </div>
            ...
            <button
              type="submit"
              className="w-full bg-[#DA291C] hover:bg-red-700 text-white font-extrabold py-4 px-6 rounded-sm text-xs uppercase tracking-wider transition-all duration-300 shadow-sm shadow-[#DA291C]/15"
            >
