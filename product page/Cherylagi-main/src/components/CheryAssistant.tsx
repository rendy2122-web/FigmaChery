...
import { MessageSquare, Send, X, Bot, Sparkles, Loader2, ArrowUpRight } from "lucide-react";
...
export default function CheryAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Halo! Saya **CHIVA** (Chery Intelligent Virtual Assistant). Ada yang ingin Anda tanyakan mengenai spesifikasi, harga, atau keunggulan teknologi dari **Chery OMODA E5**, **TIGGO 8 Pro Max**, atau **OMODA 5 GT**? Saya siap membantu Anda mengambil keputusan terbaik."
    }
  ]);
  ...
  // Pre-configured question suggestion chips
  const SUGGESTIONS = [
    "Berapa jarak tempuh OMODA E5?",
    "Bandingkan Omoda E5 & Tiggo 8",
    "Apa kelebihan LFP battery?",
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      // Beautiful self-healing fallback answer
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
  ...
  // Quick action for suggestion chips
        ...
        <div className="w-[360px] sm:w-[400px] h-[540px] bg-white border border-slate-200 rounded-sm overflow-hidden flex flex-col shadow-2xl relative">
          ...
          <div className="bg-slate-50 border-b border-slate-200 px-5 py-4 flex items-center justify-between">
            ...
            <button
              ...
              className="text-slate-400 hover:text-slate-950 p-1.5 hover:bg-slate-100 rounded-sm transition-colors"
            >
            ...
            {messages.map((msg, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`p-3.5 rounded-sm text-xs leading-relaxed font-sans ${
                      isAssistant
                        ? "bg-white text-slate-700 border border-slate-200 rounded-tl-none whitespace-pre-wrap shadow-sm"
                        : "bg-[#DA291C] text-white font-semibold rounded-tr-none shadow-sm"
                    }`}
                  >
                    {/* Render basic custom bold markdown parsing for premium look */}
          ...
          <div className="px-5 py-2.5 bg-white border-t border-slate-100 flex gap-2 overflow-x-auto whitespace-nowrap hide-scrollbar">
            {SUGGESTIONS.map((suggestion, index) => (
              <button
                ...
                className="bg-white hover:bg-slate-50 text-slate-600 border border-slate-200 hover:border-slate-300 px-3 py-1.5 rounded-sm text-[10px] font-bold tracking-wide transition-all duration-200 flex items-center gap-1 select-none shrink-0"
              >
                {suggestion}
          ...
          >
            <input
              type="text"
              placeholder="Tanyakan spesifikasi, harga, atau ADAS..."
              value={input}
            ...
            <button
              type="submit"
              disabled={!input.trim()}
              className="bg-[#DA291C] hover:bg-red-700 disabled:bg-slate-100 disabled:text-slate-300 text-white p-3.5 rounded-sm transition-all font-bold shrink-0 shadow-sm"
            >
