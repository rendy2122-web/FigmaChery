...
const dealerPhones: Record<string, string> = {
  "Cibubur": "6289527072446",
  "Makassar": "6289527072446",
  "Pare-pare": "6289527072446",
};

const dealers = ["Cibubur", "Makassar", "Pare-pare"];
...
export function BookingForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const selectedDealer = formData.get("dealer") as string;
    const fullName = formData.get("fullName") as string;
    const phone = formData.get("phone") as string;
    const email = formData.get("email") as string;
    const message = formData.get("message") as string;

    const dealerPhone = dealerPhones[selectedDealer] || "6289527072446";

    const whatsappMessage = `Halo Tim Chery ${selectedDealer}, saya ${fullName} tertarik untuk ${selectedProgram} ${selectedModel}. Berikut data saya:\n\nNama: ${fullName}\nPhone: +62 ${phone}\nEmail: ${email}${message ? `\nPesan: ${message}` : ""}\n\nMohon informasinya ya. Terima kasih.`;

    const encodedMessage = encodeURIComponent(whatsappMessage);
    const whatsappUrl = `https://wa.me/${dealerPhone}?text=${encodedMessage}`;
    ...
    >
      <div className="flex flex-col gap-4">
        <h1 className="text-h1 font-heading font-bold text-foreground">
          Tentukan Dealer Anda
        </h1>
        <p className="text-body-lg text-muted-foreground">
          Rasakan Pengalaman Berkendara Masa Depan!
        </p>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <div className="flex flex-col gap-3">
            ...
            <select id="model" required defaultValue="Chery J6" className={inputClass}>
              {models.map((model) => (
          ...
          <div className="flex flex-col gap-3">
            ...
            <select id="dealer" required defaultValue="Cibubur" className={inputClass}>
              {dealers.map((dealer) => (
